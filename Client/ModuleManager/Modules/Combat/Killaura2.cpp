...
bool packetattack2 = false;
bool antitotem2 = false; //not done
Killaura2::Killaura2() : Module("KillauraT", "Auto attack players / mobs arround u.", Category::COMBAT) {
	...
	addBoolCheck("PacketAttack", "Breaks armor faster but is weaker", &packetattack2);
	addBoolCheck("NoSwing", "NULL", &noSwing);
	addBoolCheck("AntiTotem", "NULL", &antitotem2); //using surround in the air with killaura makes you take less damage for whatever reason. we can just remove surround rendering and placements and add it under on tick. not done yet.
	addSlider<int>("TPS", "TimerSpeed", ValueType::INT_T, &timer, 0, 100);
}
std::string Killaura2::getModName() {
...
int Killaura2::getBestWeaponSlot() {
...
Vec2<float> GetRotations2(Vec3<float> playerEyePos, Vec3<float> targetPos) {
...
decltype(GetTickCount64()) startx2;  //????
bool Counterx2(double a1) {
	if ((GetTickCount64() - startx2) >= a1) {
		startx2 = GetTickCount64();
		return true;
	}
	return false;
}

void Killaura2::onNormalTick(Actor* actor) {
	auto localPlayer = mc.getLocalPlayer();
	...
	if (!targetList.empty()) {
		...
		if (TargetDis < MaxSrange) {
			...
			Vec2  rotAngle = GetRotations2(*mc.getLocalPlayer()->getPosition(), *targetList[0]->getPosition());
		...
		if (attackDelayTick >= attackDelay) {
			...
			for (int target = 0; target < mult; target++) {
				if (Mode == 1 && !packetattack2)
				{
					if (Counterx2((double)1000.f / (float)APS)) {
						for (auto& target : targetList) {
							if (target->stateVectorComponent->pos.dist(localPlayer->stateVectorComponent->pos) < ARange) {
								if (!(hurttime && target->hurtTime > 0)) {
									//InteractPacket inter(InteractAction::LEFT_CLICK, mc.getLocalPlayer()->getRuntimeID(), target->stateVectorComponent->pos.sub(Vec3<int>(0.f, 0.2f, 0.f)));
...
void Killaura2::onSendPacket(Packet* packet, bool& shouldCancel) {
